    ...
    def fetch_read_information(self, chr, start, end):
        ...
        for ef in self.bam.fetch(chr, start, end):
            ...
            if ef.get_tag("tp") != 'P':
                ...
                continue
            reads.append(ef)
            read_seq_down.append( _get_read_seq_downstream_position(ef, start) )
            read_seq_up.append( _get_read_seq_upstream_position(ef, end) )
        ...
        read_info.loc[:,'qry_seq_from_start'] = read_seq_down
        read_info.loc[:,'qry_seq_before_end'] = read_seq_up

        return({'df':read_info, 'reads': reads})


def _get_read_seq_downstream_position(alignedread, ref_position):
    """
...
def _get_read_seq_upstream_position(alignedread, ref_position):
    """
    Function to get the sequence from a given position in the reference genome
    """
    read_sequence = ''
    ...
    ## read should be aligned until the break point or 5 base pairs after.
    overlap = alignedread.get_overlap(ref_position, ref_position + 5)
    # print(overlap)
    if overlap > 0:
        ## reads that overlap the breakpoint
        # now you have two scenarios
        ...
        # reads that pass through the break point have match position before ref_position
        if aligned_pairs[0][1] < ref_position:
            qry_break_ix = [t for t in aligned_pairs if t[1] == ref_position - 1]
            assert len(qry_break_ix) > 0, 'there must be an aligned position at the break point'
            # print(qry_break_ix)
            read_sequence = qry_sequence[:qry_break_ix[0][0]]
